    ...
    >
      ...
      <div className="h-14 flex items-center gap-2 px-3 bg-linear-to-r from-red-600 to-red-700 shrink-0">
        <button onClick={() => setSidebarOpen((prev) => !prev)} className="flex items-center gap-3 w-full">
          ...
          {sidebarOpen && <div className="text-white font-semibold tracking-wide">WAMOHUB</div>}
        </button>
      </div>

      {/* Navigation */}
      <nav className="p-2 text-sm flex-1 overflow-y-auto">
        <SideLink icon={Home} label="Übersicht" active={module === "home"} onClick={() => nav("home")} />
        <SideLink icon={User} label="Consultant" active={module === "consultant"} onClick={() => nav("consultant")} />
        <SideLink icon={Building2} label="Organisation" active={module === "organisation"} onClick={() => nav("organisation")} />
        <SideLink icon={Briefcase} label="Projekte" active={module === "projekte"} onClick={() => nav("projekte")} />
        {canAccessModule(activeUser, "mentor") && (
          <SideLink icon={UsersIcon} label="Mentor" active={module === "mentor"} onClick={() => nav("mentor")} />
        )}
        <SideLink icon={GraduationCap} label="Academy" active={module === "academy"} onClick={() => nav("academy")} />
        <SideLink icon={StickyNote} label="Notizen" active={module === "notizen"} onClick={() => nav("notizen")} />
        <SideLink icon={ExternalLink} label="Sonstiges" active={module === "sonstiges"} onClick={() => nav("sonstiges")} />

        {/* Quick Actions */}
        <div className="mt-4 pt-4 border-t border-gray-700">
          <div className="text-xs text-gray-300 mb-2 px-3">Quick Actions</div>
          <button
        ...
        {canAccessModule(activeUser, "admin") && (
          <div className="mt-2 pt-2 border-t border-gray-700">
            <SideLink icon={Shield} label="Benutzer & Rollen" active={module === "rbac"} onClick={() => nav("rbac")} />
            <SideLink icon={CheckCircle} label="Approvals" active={module === "approvals"} onClick={() => nav("approvals")} />
            <SideLink icon={Settings} label="Admin/Settings" active={module === "admin"} onClick={() => nav("admin")} />
          </div>
        )}
      </nav>
    </div>
